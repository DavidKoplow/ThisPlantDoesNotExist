...
scientific_names = get_attributes("Scientific Name")
...
#for i in range(3, 10):
    #prefix = "A plant "
    scientific_name = f"{scientific_names[i]} is "
    ...
    flower_color = f"with {isknown(flower_colors[i])} flower color. " if not isnan(str(flower_colors[i])) else ""
    s_category = area + category + kingdom + flower_color + '.\n'
    ...
    L = [s_category] 
        #+ [duration, habit, growth_period, fall_conspicuous] + [flower_color, flower_conspicuous] + \
